...
let renderCard = function (product) {
	...
	let h2 = makeElement('h2', 'good__description', product.text);
	listItem.appendChild(h2);
	let picture = makeElement('img', 'good__image');
	picture.alt = product.text;
	picture.src = product.imgUrl;
	listItem.appendChild(picture);
	let price = makeElement('p', 'good__price', product.price + '₽/кг');
	...
	if (product.isHit) {
		...
		let special = makeElement('p', 'good__special-offer', product.specialOffer);
...
let productList = document.querySelector('.goods');

for (i = 0; i < cardsData.length; i++) {
	let productItem = renderCard(cardsData[i]);
